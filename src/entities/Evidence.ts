...
export type EvidenceType = 
    | 'claw_marks'      // Bear, big cats
    | 'footprints'      // All animals
    | 'feathers'        // Birds
    | 'fur_tuft'        // Mammals
    | 'eaten_fish'      // Bear, seals
    | 'chewed_carrot'   // Rabbit
    | 'cracked_nut'     // Squirrel
    | 'droppings'       // Various
    | 'nest'            // Birds
    | 'burrow'          // Rabbit, fox, meerkat
    | 'scratch_marks'   // Various
    | 'tracks'          // General tracks
    | 'shell_fragments' // Turtle, crab
    | 'shed_skin'       // Snake, lizard
    | 'web'             // Spider (future)
    | 'dig_marks';      // Meerkat, fox

export interface EvidenceData {
    type: EvidenceType;
    name: { nl: string; en: string };
    description: { nl: string; en: string };
    linkedAnimal: string;  // Animal ID this evidence belongs to
...
export class Evidence {
    private id: string;
    private data: EvidenceData;
    private position: { x: number; y: number };
    private discovered: boolean = false;
    private collected: boolean = false;
    private animationFrame: number = 0;
    private glowIntensity: number = 0;
    private size = { width: 48, height: 48 };

    constructor(data: EvidenceData, x: number, y: number) {
        this.data = data;
        this.position = { x, y };
    ...
    public renderInteractionPrompt(ctx: CanvasRenderingContext2D, cameraX: number, cameraY: number, language: string): void {
        if (this.collected) return;
...
export function generateEvidenceForAnimal(animalId: string, animalX: number, animalY: number): Evidence[] {
