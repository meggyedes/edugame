...
type SelectCallback = (zoneId: string, levelId: string) => void;
...
export class ZoneSelectScreen {
    ...
    private hoveredLevel: string | null = null;
    private animationTimer: number = 0;
    private showLevelSelect: boolean = false;
    private onSelectCallback: SelectCallback | null = null;
    ...
    public setOnSelect(callback: SelectCallback): void {
    ...
    private updateZoneHover(mouseX: number, mouseY: number): void {
    ...
    private updateLevelHover(mouseX: number, mouseY: number): void {
    ...
    private getZonePosition(index: number): { x: number; y: number } {
    ...
    private renderCompass(ctx: CanvasRenderingContext2D, x: number, y: number): void {
        ctx.save();
        ctx.translate(x, y);
    ...
    private renderZoneMap(ctx: CanvasRenderingContext2D): void {
        // Title
        ctx.fillStyle = '#FFD700';
        ...
        ctx.font = '16px Arial';
        ctx.fillText(
            `${progress.zonesCompleted.length}/5 ${this.i18n.t('zones_completed')}`,
    ...
    private renderZoneNode(ctx: CanvasRenderingContext2D, zone: Zone, index: number): void {
        const pos = this.getZonePosition(index);
    ...
    private renderLevelSelect(ctx: CanvasRenderingContext2D): void {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(
            `${this.selectedZone.icon} ${this.selectedZone.name[this.i18n.getLanguage()]}`,
            this.canvasWidth / 2, panelY + 40
        );
        ...
        ctx.font = '14px Arial';
        ctx.fillText(
            this.selectedZone.description[this.i18n.getLanguage()],
            this.canvasWidth / 2, panelY + 65
        );
    ...
    private renderLevelItem(ctx: CanvasRenderingContext2D, level: MiniLevel, x: number, y: number, width: number): void {
    ...
    private renderBackButton(ctx: CanvasRenderingContext2D, x: number, y: number): void {
