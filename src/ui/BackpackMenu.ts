...
export type BackpackItemId = 'camera' | 'journal' | 'map' | 'microphone' | 'binoculars' | 'notebook';
...
export class BackpackMenu {
    ...
    // Position (top-left)
    private backpackX: number = 20;
    private backpackY: number = 20;
    private backpackSize: number = 60;
    
    // Dropdown dimensions
    private dropdownWidth: number = 200;
    ...
    // Callbacks
    private onCameraClick: (() => void) | null = null;
    private onJournalClick: (() => void) | null = null;
    private onMapClick: (() => void) | null = null;
    ...
    // Set callbacks
    public setOnCameraClick(callback: () => void): void {
        this.onCameraClick = callback;
    }

    public setOnJournalClick(callback: () => void): void {
        this.onJournalClick = callback;
    }

    public setOnMapClick(callback: () => void): void {
        this.onMapClick = callback;
    }

    public unlockItem(itemId: BackpackItemId): void {
    ...
    public isClickOnBackpack(x: number, y: number): boolean {
    ...
    private renderBackpackButton(ctx: CanvasRenderingContext2D, lang: string): void {
        ...
        const gradient = ctx.createLinearGradient(
            this.backpackX, this.backpackY, 
            this.backpackX, this.backpackY + this.backpackSize
        );
    ...
    private renderBackpackIcon(ctx: CanvasRenderingContext2D, cx: number, cy: number): void {
        // Main bag body
    ...
    private renderDropdown(ctx: CanvasRenderingContext2D, lang: string): void {
        ctx.save();
    ...
    private renderItem(ctx: CanvasRenderingContext2D, item: BackpackItem, index: number, dropdownY: number, lang: string): void {
        ...
        ctx.textAlign = 'left';
        ctx.fillStyle = isLocked ? '#666' : '#FFF';
        ctx.fillText(item.name[lang as 'nl' | 'en'], this.backpackX + 55, itemY + 18);
        ...
        const desc = isLocked ? 
            ...
            item.description[lang as 'nl' | 'en'];
