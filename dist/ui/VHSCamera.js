...
export class VHSCamera {
    constructor(canvasWidth, canvasHeight) {
        ...
        this.maxCameraOffset = 300; // Max distance camera can move from player
    ...
    renderLCDOverlay(ctx, lcdX, lcdY, lcdW, lcdH) {
    ...
    getFocusIndicatorColor() {
        const focusDiff = Math.abs(this.focusLevel - this.targetFocus);
        if (focusDiff < 0.1)
            return '#00FF00'; // Green = good focus
        if (focusDiff < 0.25)
