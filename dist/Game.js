...
import { LevelManager } from './zones/LevelManager.js';
...
import { TilesetRenderer } from './world/TilesetRenderer.js';
...
export class Game {
    constructor() {
        ...
        this.mouseY = 0;
        // Interaction state
        this.nearbyAnimal = null;
        this.nearbyEvidence = null; // NEW: Evidence system
        this.showAnimalInfo = false;
        this.animalInfoTimer = 0;
        this.currentAnimalFact = '';
        this.lastBiome = 'beach';
        // World dimensions - MUCH LARGER WORLD!
        this.WORLD_WIDTH = 8000;
        this.WORLD_HEIGHT = 6000;
        // Get canvas
        ...
        this.biomeSelectScreen.setOnBiomeSelect((biomeId) => {
        ...
        this.zoneSelectScreen.setOnSelect((zoneId, levelId) => {
    ...
    updateCanvasSize() {
        ...
        if (this.progressScreen)
            this.progressScreen.updateSize(this.width, this.height);
        if (this.photoGallery)
            this.photoGallery.updateSize(this.width, this.height);
        if (this.worldMap)
            this.worldMap.updateSize(this.width, this.height);
        if (this.backpack)
            this.backpack.updateSize(this.width, this.height);
        if (this.zoneSelectScreen)
            this.zoneSelectScreen.updateSize(this.width, this.height);
        if (this.journal)
            this.journal.updateSize(this.width, this.height);
        if (this.vhsCamera)
            this.vhsCamera.updateSize(this.width, this.height);
        if (this.backpackMenu)
            this.backpackMenu.updateSize(this.width, this.height);
        if (this.biomeSelectScreen)
            this.biomeSelectScreen.updateSize(this.width, this.height);
    }
    setupFullscreen() {
    ...
    startLevel(zoneId, levelId) {
        ...
        this.gameCamera.setPosition(this.player.getCenter().x - this.width / 2, this.player.getCenter().y - this.height / 2);
    ...
    handleVHSPhotoCapture() {
        if (!this.vhsCamera.isActiveMode() || !this.inBiomeMode || !this.currentBiomeLevel)
            return;
        const crosshair = this.vhsCamera.getWorldCrosshairPosition(this.gameCamera.getX(), this.gameCamera.getY());
        ...
        for (const evidence of this.currentBiomeLevel.getEvidence()) {
            ...
            if (distance < 60) {
                // Photo of evidence!
                const photo = this.vhsCamera.takePhoto(this.canvas, 'evidence', evidence.getData().type, this.currentBiomeLevel.getLevelData().biome, this.gameCamera.getX(), this.gameCamera.getY());
                if (photo) {
        ...
        for (const animal of this.currentBiomeLevel.getAnimals()) {
            ...
            if (distance < size.width / 2 + 50) {
                // Photo of animal!
                const photo = this.vhsCamera.takePhoto(this.canvas, 'animal', animal.getId(), this.currentBiomeLevel.getLevelData().biome, this.gameCamera.getX(), this.gameCamera.getY());
                if (photo) {
                    this.audio.playPhotoCapture();
        ...
        this.vhsCamera.takePhoto(this.canvas, 'evidence', 'missed', 'beach', this.gameCamera.getX(), this.gameCamera.getY());
        this.audio.playPhotoCapture();
    }
    // NEW: Handle biome level completion
    onBiomeLevelComplete() {
        const lang = this.i18n.getLanguage();
        ...
        setTimeout(() => {
            this.hud.showNotification(`🎉 ${levelData.name[lang]} ${lang === 'nl' ? 'voltooid!' : 'completed!'}`, 5);
    ...
    toggleLanguage() {
        const newLang = this.i18n.getLanguage() === 'nl' ? 'en' : 'nl';
    ...
    updateGameplay(deltaTime, inputState) {
        ...
        if (this.vhsCamera.isActiveMode()) {
            ...
            let camDx = 0, camDy = 0;
            if (fullInputState.up)
                camDy = -1;
            if (fullInputState.down)
                camDy = 1;
            if (fullInputState.left)
                camDx = -1;
            if (fullInputState.right)
                camDx = 1;
            if (camDx !== 0 || camDy !== 0) {
    ...
    renderBiomeBackground(biomeType, cameraX, cameraY) {
    ...
    renderEvidenceMarker(evidence, screenX, screenY) {
    ...
    renderBiomeAnimal(animal, screenX, screenY) {
    ...
    renderUIButtons() {
        ...
        this.renderIconButton(backpackBounds.x, backpackBounds.y, backpackBounds.width, backpackBounds.height, 'backpack', 'B');
        // Map button
        const mapBounds = this.getMapButtonBounds();
        this.renderIconButton(mapBounds.x, mapBounds.y, mapBounds.width, mapBounds.height, 'map', 'M');
        // Journal button (for photos)
        const journalBounds = this.getJournalButtonBounds();
        this.renderIconButton(journalBounds.x, journalBounds.y, journalBounds.width, journalBounds.height, 'journal', 'J');
    }
    renderIconButton(x, y, w, h, icon, key) {
        // Check if hovered
