...
export class LevelRenderer {
    ...
    generateDecorations() {
        if (!this.currentZone || !this.currentLevel)
            return;
        this.decorations = [];
    ...
    generateJungleDecorations(width, height) {
        // Dense trees
        for (let i = 0; i < 80; i++) {
    ...
    generateDesertDecorations(width, height) {
        // Cacti
        for (let i = 0; i < 40; i++) {
            this.decorations.push({
                x: Math.random() * width,
    ...
    generateArcticDecorations(width, height) {
        // Ice formations
        for (let i = 0; i < 35; i++) {
            this.decorations.push({
                x: Math.random() * width,
    ...
    generateOceanDecorations(width, height) {
        // Coral reefs
        for (let i = 0; i < 45; i++) {
            this.decorations.push({
                x: Math.random() * width,
    ...
    generateSavannahDecorations(width, height) {
        // Acacia trees
        for (let i = 0; i < 25; i++) {
            this.decorations.push({
                x: Math.random() * width,
    ...
    renderBackground(ctx, viewX, viewY, viewWidth, viewHeight) {
        if (!this.currentZone || !this.currentLevel)
            return;
    ...
    renderDecorations(ctx, viewX, viewY, viewWidth, viewHeight) {
        if (!this.currentZone)
            return;
        // Sort by Y for proper layering
        const visibleDecorations = this.decorations
            .filter(d => d.x >= viewX - 100 && d.x <= viewX + viewWidth + 100 &&
    ...
    renderInteractives(ctx, interactives, viewX, viewY) {
    ...
    renderPanoramaPoints(ctx, points, viewX, viewY) {
