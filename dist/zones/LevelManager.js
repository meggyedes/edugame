...
export class LevelManager {
    ...
    checkPanoramaPoint(playerX, playerY) {
        if (!this.currentState)
            return null;
        for (let i = 0; i < this.currentState.panoramaPoints.length; i++) {
            ...
            if (point.discovered)
                continue;
            const dist = Math.sqrt(Math.pow(playerX - point.x, 2) +
    ...
    interact(objectId) {
        ...
        switch (obj.type) {
            case 'button':
                ...
                if (obj.targetId) {
                    const target = this.currentState.interactives.find(i => i.type === 'gate' || i.id === obj.targetId);
                    if (target) {
                        target.state = 'active';
    ...
    findNearbyInteractive(playerX, playerY, range = 60) {
        if (!this.currentState)
            return null;
        for (const obj of this.currentState.interactives) {
            if (obj.isCompleted && obj.type !== 'boat')
                continue;
