...
export class Animal {
    ...
    render(ctx, cameraX, cameraY) {
        ...
        if (this.spriteLoader.isReady()) {
            const spriteDrawn = this.spriteLoader.drawSprite(ctx, this.id, screenX - this.size.width / 2, screenY - this.size.height / 2 + bounce, 5.625, // Scale up for maximum visibility (7.5x from original)
            this.facingLeft);
            if (spriteDrawn) {
    ...
    renderPolarBear(ctx, x, y, bounce) {
        const cx = x + 16;
        const cy = y + 16 + bounce;
        // Body
    ...
    // ==================== NEW TUNDRA ANIMALS ====================
    renderArcticFox(ctx, x, y, bounce) {
        const cx = x + 18;
        const cy = y + 16 + bounce;
        // Body
    ...
    getName(language) { return this.name[language]; }
    getDescription(language) { return this.description[language]; }
    getBiome() { return this.biome; }
    getPosition() { return { ...this.position }; }
    getSize() { return { ...this.size }; }
    getPoints() { return this.points; }
    isDiscovered() { return this.discovered; }
    isPhotographed() { return this.photographed; }
    getFacts(language) { return this.facts[language]; }
    discover() { this.discovered = true; }
    photograph() { this.photographed = true; }
